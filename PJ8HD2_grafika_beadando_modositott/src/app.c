...
while (SDL_PollEvent(&event)) {
	switch (event.type) {
        ...
        case SDL_MOUSEBUTTONDOWN:
			...
			set_plane_speed_y(&(app->scene.car), -1);
			}
...
void update_app(App* app)
{
	...
	if(app->scene.car.p_position.y > 30.0)
    {
        set_plane_speed_y(&(app->scene.car),0);
		app->scene.car.p_position.x=30.0;
		app->scene.car.p_position.y=-10;
		set_plane_speed_x(&(app->scene.car),-1);

        set_plane_rotation_z(&(app->scene.car),1.0);
        set_plane_angle(&(app->scene.car),-90.0);
         
    
    } 
		if(app->scene.car.p_position.x < -30.0)
    {
        set_plane_speed_x(&(app->scene.car),0);
		app->scene.car.p_position.x=3.0;
		app->scene.car.p_position.y=-20;
		set_plane_speed_y(&(app->scene.car),-1);

        set_plane_rotation_z(&(app->scene.car),1.0);
        set_plane_angle(&(app->scene.car),-180.0);
		 ...
		 if(app->scene.car.position.x >= 11)
    {
		...
		app->scene.car.position.x-=0.3;
			 ...
			 if(app->scene.car.position.x >10.88)
    {
	...
	move_plane(&(app->scene.car), elapsed_time);
}
